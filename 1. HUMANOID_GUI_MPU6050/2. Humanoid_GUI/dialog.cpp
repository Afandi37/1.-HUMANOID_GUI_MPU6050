    ...
    , arduino(new QSerialPort(this))
{
    ...
    quickWidget->setSource(QUrl(QStringLiteral("qrc:/Main3D.qml")));
    ...
    connect(timer, &QTimer::timeout, this, &Dialog::readSerialData);
    timer->start(100); // Membaca setiap 100 ms
...
//             ui->textEdit_serialMonitor->append(mpuData); // Tambahkan baris ini untuk menampilkan data di monitor serial
//             //ui->textEdit_mpuData1->append(mpuData); // Pastikan Anda memiliki QTextEdit dengan objectName "textEdit_mpuData"
...
void Dialog::readSerialData()
{
    ...
    while (serialData.contains('\n')) {
        ...
        // Regular expression to match X, Y, Z values
        QRegularExpression regex(R"(X:\s*(-?\d+\.\d+),\s*Y:\s*(-?\d+\.\d+),\s*Z:\s*(-?\d+\.\d+))");
        QRegularExpressionMatch match = regex.match(dataStr);

        if (match.hasMatch()) {
            QString roll = match.captured(2);  // X value
            QString pitch = match.captured(3); // Y value
            QString yaw = match.captured(1);   // Z value

            // Konversi dari QString ke float
            float pitchValue = pitch.toFloat();

            // Membalik tanda pitch
            pitchValue = -pitchValue;

            // Konversi kembali ke QString jika dibutuhkan
            pitch = QString::number(pitchValue);

            // Emit the signal with roll, pitch, yaw
            emit dataUpdated(roll, pitch, yaw);

            // Display the values in the respective QTextEdits
            ui->textEdit_Roll->append(roll);
            ui->textEdit_Pitch->append(pitch);
            ui->textEdit_Yaw->append(yaw);
        } else {
            qDebug() << "Format data tidak sesuai dengan yang diharapkan.";
        }
    }

    //////////////////////program untuk membaca serial monitor////////////////////////
    // while (arduino->canReadLine()) {
    //     QByteArray line = arduino->readLine();
    //     QString dataStr = QString::fromUtf8(line).trimmed();
    //     qDebug() << "Data diterima:" << dataStr;

    //     // Tampilkan data di QTextEdit
    //     if (!dataStr.isEmpty()) {
    //         ui->textEdit_serialMonitor->append(dataStr);
    //         qDebug() << "Menambahkan ke QTextEdit:" << dataStr;
    //     }
    // }
}
