...
#define BUFFER_SIZE 50   // Set a maximum buffer size for incoming data

char buffer[BUFFER_SIZE]; // Array to store incoming data
int index = 0;            // Index for tracking position in buffer
...
void setup() {
 ...
 // SerialUSB.begin(115200);       // Untuk debugging di Serial Monitor
  Serial2.begin(115200);     // Kecepatan yang sama dengan Serial1 pada transmitter
...
{ 
  ...
  { 
   ...
   while (Serial2.available() > 0) {
    char incomingChar = Serial2.read();  // Read each character from Serial2

    // If newline character is received or buffer is full, process the buffer
    if (incomingChar == '\n' || index >= BUFFER_SIZE - 1) {
      buffer[index] = '\0'; // Null-terminate the character array
      SerialUSB.print("->");
      SerialUSB.println(buffer); // Display the received data on Serial

      // Reset the index for the next message
      index = 0;
    } else {
      // Store character in buffer and move to next position
...
void berdiri_tegak()
{
 ...
 //tangankanandankiri
  ...
  delay(2000);
  Dxl.setPosition(ID_NUM3,550,50);
...
void jalan_ditempat()
{
    ...
    Dxl.setPosition(ID_NUM3,515,100);
    ...
    Dxl.setPosition(ID_NUM3,415,300);
...
void kuda_kuda()
{
 ...
 //tangan kanan
  ...
  delay(2000);
  Dxl.setPosition(ID_NUM3,515,50);
...
void melambaikan()
{
  ...
  Dxl.setPosition(ID_NUM3,415,300);
  delay(1000);
  Dxl.setPosition(ID_NUM3,515,300);
   delay(1000);
  Dxl.setPosition(ID_NUM3,415,300);
  delay(1000);
  Dxl.setPosition(ID_NUM3,515,300);
  ...
  Dxl.setPosition(ID_NUM3,415,300);
  delay(1000);
  Dxl.setPosition(ID_NUM3,615,300);
  delay(1000);
  Dxl.setPosition(ID_NUM1,515,100);
  Dxl.setPosition(ID_NUM3,615,100);
  Dxl.setPosition(ID_NUM5,15,100);
